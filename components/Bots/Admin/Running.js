...
const selectStyles = {
  container: (provided, {selectProps: {width}}) => ({...provided, width: width, minWidth: '150px', color: '#fff'}),
...
const RunningBots = ({
  ...
  const choiceRestoreBot = instance => {
    restoreBot(instance.id).then(() => notify({
  ...
  const changeBotInstanceStatus = (option, id) => {
    updateAdminRunningBot(id, {status: option.value}).then(() => notify({
      type: NOTIFICATION_TYPES.INFO,
  ...
  const renderRow = (botInstance, idx) => <Tr key={idx} disabled={botInstance.status === 'pending'}>
      ...
      <td>
        ...
        <div title={'Copy Instance'} onClick={() => choiceCopyInstance(botInstance)}>
          <A>Clone</A>
        </div>
        <div disabled={botInstance.status === 'terminated'} title={'Download PEM'} type={'success'}
          ...
          <Filters>
            ...
            <ListFilter options={FILTERS_LIST_OPTIONS}
                        onChange={({value}) => {
                          setFilterList(value);
                          adminGetRunningBots(
                            {page, limit, list: value, sort: order.field, order: order.value, search});
                        }}
            />
