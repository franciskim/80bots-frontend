...
import { getPosts, addPost, updatePost, deletePost } from '../../store/cms/actions';
    ...
    <td>
      ...
      <IconButton title={'Edit Post'} type={'primary'}>
        <Link href={'/admin/cms/posts/[id]/edit'} as={`/admin/cms/posts/${post.id}/edit`}>
          <A><Icon name={'edit'} color={theme.colors.white}/></A>
        </Link>
      </IconButton>
    ...
    <>
      <AddButtonWrap>
        <AddButton type={'primary'}>
          <Link href={'/admin/cms/add'}><A>Add Post</A></Link>
        </AddButton>
      </AddButtonWrap>
      <Container>
        <CardBody>
          <Filters>
            <LimitFilter onChange={({ value }) => {setLimit(value); getPosts({ page, limit: value, sort: order.field, order: order.value, search }); }}/>
            <SearchFilter onChange={( value ) => { searchPosts(value); }}/>
          </Filters>
          ...
          <Paginator total={total} pageSize={limit} onChangePage={(page) => { setPage(page); getPosts({ page, limit, sort: order.field, order: order.value, search }); }}/>
