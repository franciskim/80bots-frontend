...
export const formatTimezone = (scheduleDate, timezone ) => {
  const date = dayjs(scheduleDate).format("YYYY-MM-DD H:mm");
  console.log(date);
  const formatCurrentTime = moment.tz(`${date}`, `${timezone}`);
  console.log(formatCurrentTime.tz('UTC').format('YYYY-MM-DD HH:mm'));
};