...
import { listenForWhisper, stopListeningForWhisper } from '../socket/actions'
import { addScriptNotification, updateLastNotification } from '../bot/actions'
    ...
    return (next) => (action) => {
      ...
      const listener = {
        ...
        subscribe: () => {
          ...
          return dispatch(
            listenForWhisper(channel, signal, (data) => {
              console.log({ signal, data })
              if (signal === 'notification') {
                dispatch(addScriptNotification(data))
                dispatch(
                  updateLastNotification(data.instanceId, data.notification)
                )
              }
            })
          )
      ...
      switch (action.type) {
        case OPEN_SCRIPT_NOTIFICATION: {
          listener.subscribe()
          const isListener = listeners.some((item) => item.channel === channel)
          if (!isListener) {
